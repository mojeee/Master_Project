...
C
      ...
      RLNP = RCKWRK(NcRU) * DLOG(P / RCKWRK(NcPA))
C
      ABML = 0.d0
      DO 100 K = 1, NKK
         ABML = ABML + X(K) * ( RCKWRK(NcK2 + K - 1) - T *
     1          (RCKWRK(NcK1 + K - 1) - RCKWRK(NcRU)
     2          * DLOG(MAX(X(K),SMALL)) - RLNP) )
  100 CONTINUE
C
      RETURN
      END
...
C
     ...
     2             ( RCKWRK(NcK1 + K - 1) -
     3               RCKWRK(NcRU)*
     4               DLOG(MAX(RCKWRK(NcK3 + K - 1),SMALL)) - RLNP))
  100 CONTINUE
C
...
C
     ...
     1             ICKWRK(IcNU), ICKWRK(IcNK), NFAL, ICKWRK(IcFL),
     2             ICKWRK(IcFO), ICKWRK(IcKF), NFAR, RCKWRK(NcFL),
     3             NTHB, ICKWRK(IcTB), ICKWRK(IcKN), RCKWRK(NcKT),
...
C
      DATA RU,RUC,PA /8.314d7, 1.987d0, 1.01325d6/
C
...
C
   ...
   15 FORMAT (/1X,' CKLIB:  Chemical Kinetics Library',
     1        /1X,'         CHEMKIN-II Version 3.9, Apr. 1992',
C*****precision > double
...
C
      ...
      IF (.NOT.IOK .OR. .NOT.ROK .OR. .NOT.COK) STOP
C
      REWIND LINC
...
C
      DO 10 I = 1, II
         RCKWRK(NcCO + (I-1)*(NPAR+1) + NPAR) = 1.d0
   10 CONTINUE
C
...
C
     ...
     1   (RNUM.GT.0.d0 .AND. RNUM.LT.SMALL) .OR.
     2   (RNUM.LT.0.d0 .AND. RNUM.GT.SMALL)) THEN
         KERR = .TRUE.
         RETURN
      ENDIF
C
...
C
   25 CONTINUE
      IF (IE .LT. 0) THEN
         RVAL = VAL * 10.d0**(ABS(IE) - 1) * 1000.d0
      ELSEIF (IE .GT. 0) THEN
         RVAL = VAL * 10.d0**(-IE + 1) * 10.d0
      ELSE
         RVAL = VAL * 100.d0
      ENDIF
      IF (RVAL.LT.100.d0 .OR. RVAL.GE.1000.d0) THEN
         IF (RVAL .LT. 100.d0) IE = IE - 1
         IF (RVAL .GE. 1000.d0)IE = IE + 1
         GO TO 25
      ELSE
         IVAL = NINT (RVAL)
...
C
      DO 20 I = 1, II
         CTB(I) = 1.d0
         RKF(I) = PAR(1,I) * DEXP(PAR(2,I)*ALOGT - PAR(3,I)/T)
   20 CONTINUE
C
...
C
      DO 25 N = 1, NLAN
         I = ILAN(N)
         TFAC = PLT(1,N)/T**(1.d0/3.d0) + PLT(2,N)/T**(2.d0/3.d0)
         RKF(I) = RKF(I) * DEXP(TFAC)
   25 CONTINUE
C
   ...
   40     CONTINUE
C
          EQK(I) = DEXP(DMIN1(SUMSMH,EXPARG))
   50 CONTINUE
C
      PFAC = PATM / (RU*T)
      DO 60 I = 1, II
...
C
      DO 70 N = 1, NREV
         I = IREV(N)
         RKR(I) = RPAR(1,N) * DEXP(RPAR(2,N)*ALOGT - RPAR(3,N)/T)
         EQK(I) = RKF(I)/RKR(I)
   70 CONTINUE
C
...
C
      DO 75 N = 1, NRLT
         I = IRLT(N)
         TFAC = RPLT(1,N)/T**(1.d0/3.d0) + RPLT(2,N)/T**(2.d0/3.d0)
         RKR(I) = RKR(I) * DEXP(TFAC)
         EQK(I) = RKF(I)/RKR(I)
   75 CONTINUE
C
...
C
               ...
               FC = ((FPAR(4,N)*DEXP(-FPAR(5,N)/T) + DEXP(-T/FPAR(6,N)))
...
C
               FCENT = (1.d0-FPAR(4,N)) * DEXP(-T/FPAR(5,N))
...
C
      DO 20 I = 1, II
         RKFT(I) = PAR(1,I) * DEXP(PAR(2,I)*ALOGT - PAR(3,I)/T)
   20 CONTINUE
C
C        Landau-Teller reactions
C
...
C
      DO 70 N = 1, NREV
         I = IREV(N)
         RKRT(I) = RPAR(1,N) * DEXP(RPAR(2,N)*ALOGT - RPAR(3,N)/T)
         EQK(I)  = RKFT(I)/RKRT(I)
   70 CONTINUE
C
C     if reverse Landau-Teller parameters have been given:
...
C
     ...
     1                   NFAL, IFAL, IFOP, KFAL, NFAR, FPAR, NTHB, ITHB,
     2                   NTBS, AIK, NKTB, RKFT, RKRT, RKF, RKR, CTB)
C
C  START PROLOGUE
C
C  SUBROUTINE CKRATX (II, KK, MAXSP, MAXTB, T, C, NSPEC, NU, NUNK,
C 1                   NFAL, IFAL, IFOP, KFAL, NFAR, FPAR, NTHB, ITHB,
C 2                   NTBS, AIK, NKTB, RKFT, RKRT, RKF, RKR, CTB)
C
C  END PROLOGUE
C
C*****precision > double
...
C*****END precision > single
C
      DIMENSION C(*), NSPEC(*), NU(MAXSP,*), NUNK(MAXSP,*),
...
C----------------------------------------------------------------------C
C
      SUBROUTINE CKSAVE (LINC, LOUT, LSAVE, ICKWRK, RCKWRK, CCKWRK)
C
C  START PROLOGUE
C
C  SUBROUTINE CKSAVE (LINC, LOUT, LSAVE, ICKWRK, RCKWRK, CCKWRK)
...
C
      ...
      REWIND LINC
C
      CALL CKLEN (LINC, LOUT, LENI, LENR, LENC)
      NPOINT = 63
      WRITE (LSAVE)   NPOINT, VERS,   PREC,   LENI,   LENR,   LENC,
     *                NMM , NKK , NII , MXSP, MXTB, MXTP, NCP , NCP1,
     1                NCP2, NCP2T,NPAR, NLAR, NFAR, NLAN, NFAL, NREV,
     2                NTHB, NRLT, NWL,  IcMM, IcKK, IcNC, IcPH, IcCH,
     3                IcNT, IcNU, IcNK, IcNS, IcNR, IcLT, IcRL, IcRV,
...
C
      RETURN
  999 CONTINUE
      WRITE (LOUT, *) ' Error reading Chemkin linking file...'
      KERR = .TRUE.
      END
C
C----------------------------------------------------------------------C
...
C
     ...
     1          RCKWRK(NcRU)*DLOG(MAX(X(K),SMALL)) - RLNP )
  100 CONTINUE
C
      RETURN
      END
...
C
     ...
     1             ( RCKWRK(NcK1 + K - 1)
     2             - RCKWRK(NcRU) *
     3               DLOG(MAX(RCKWRK(NcK2 + K - 1),SMALL)) - RLNP)
  100 CONTINUE
C
      CALL CKMMWY (Y, ICKWRK, RCKWRK, WTM)
      SBMS = SUM / WTM
         ...
         DO 80 N = 1, NWL
            IF (ICKWRK(IcWL+N-1) .EQ. I) THEN
               ...
               IF (LT+3 .GT. ILEN) THEN
                  ...
                  RETURN
               ENDIF
               IF (J.EQ.1.AND.W.LT.0.d0 .OR. J.EQ.2.AND.W.GT.0.d0)
     1             ISTR(LT+1:) = '+HV'
            ENDIF
   80    CONTINUE
C
